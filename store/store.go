...
func (s *Store) Put(key string, value []byte) error {
	...
	// 2. Only after disk acknowledges it, apply to Memory
	s.data[key] = value
	return nil
}
...
// GetEntriesAfter replays the WAL and returns all entries with an index
// greater than afterIndex. This is used by the leader to serve missing
// entries to recovering followers.
func (s *Store) GetEntriesAfter(afterIndex uint64) ([]LogEntry, error) {
	s.mu.RLock()
	defer s.mu.RUnlock()

	allEntries, err := s.wal.Replay()
	if err != nil {
		return nil, fmt.Errorf("failed to replay WAL for sync: %v", err)
	}
