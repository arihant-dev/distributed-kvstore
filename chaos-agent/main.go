...
func main() {
	...
	provisioner := NewNodeProvisioner(engine)
	go provisioner.Start()

	h := newHandler(engine, provisioner)
	...
	fmt.Println("  GET  /chaos/cluster          (cluster health status)")
	fmt.Println("  POST /chaos/auto-provision   {\"enabled\": true/false}")
	fmt.Println()
